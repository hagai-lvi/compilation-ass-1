...
(define cut-allign-middle (lambda (str output-length)
	(let* ( 	(total-cut (+ 2 (- (string-length str) output-length)))
				(cut-right (floor (/ total-cut 2)))
				(cut-left (- total-cut cut-right))
				(sub (substring str cut-left  (- (string-length str)cut-left))))
	;(display total-cut)
	;(display cut-right)
	;(display cut-left)
	(display sub)
	...
	;    (list->string `(,unicode-char-overflow-left ,@(split-list (string->list str) (- output-length 2) (lambda(left right) left))  ,unicode-char-overflow-right)))
)))
